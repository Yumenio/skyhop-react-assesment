  ...
  return (
    <div>
      ...
      <div className='flex flex-col items-center sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-4 mt-2'>
        <div
          className={`rounded-full w-16 h-8 flex bg-${!toggleOn ? 'skyhopBlue' : 'skyhopBlue'} p-1 cursor-pointer mb-2 sm:mb-0`}
          onClick={handleToggle}
        >
          <div
            className={`w-6 h-6 rounded-full bg-white transform transition-transform ${toggleOn ? 'translate-x-8' : ''}`}
          ></div>
        </div>
